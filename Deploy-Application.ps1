...
Try {
	...
	[string]$appVendor = 'Devolutions'
	[string]$appName = 'Remote Desktop Manager'
	[string]$appVersion = '2022.2.20.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.1.3'
	[string]$appScriptDate = '2022.08.11'
	...
	$CloseApps = "RemoteDesktopManagerFree=Remote Desktop Manager"
	...
	$64bitAppInstallName = "Remote Desktop Manager"
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\Setup.RemoteDesktopManagerFree.2022.2.20.0.msi"
	# Application  install parameters.
	...
	$64bitAppUninstallName = "Remote Desktop Manager"
	# Application uninstall path.
	$64bitAppUninstallPath = "$PSScriptRoot\Files\Setup.RemoteDesktopManagerFree.2022.2.20.0.msi"
	# Application  install parameters.
	$64bitAppUninstallParam = "/quiet"
	...
	$MenuShortcutPath = "$Env:ProgramData\Microsoft\Windows\Start Menu\Programs\Remote Desktop Manager Free\Remote Desktop Manager Free (RDM).lnk"

	# Installed application location
	$64bitAppInstallLocation = "$Env:ProgramFiles (x86)\Devolutions"

	#Application data location
	$DataLocation = "$env:LOCALAPPDATA\Devolutions"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		## Check if previous prerequisite MSI versions of application are installed.
		# If MSI is installed, uninstall previous MSI versions.
		# If uninstall failed, log results. Exit script. 
		# Else, log results from check.
		$RemoveAppNamesMSICheck = Get-InstalledApplication -Name "$64bitAppUninstallName"
		If ($null -ne $RemoveAppNamesMSICheck) {
			# Uninstall previous MSI versions of application(s).
			Try {  
				Execute-MSI -Action Uninstall -Path "$64bitAppUninstallPath" -Parameters "$64bitAppUninstallParam"
			}
			# If uninstall failed, log results. Exit script.
			Catch [System.Exception] {
				Write-Log -Message "Uninstaling previous MSI versions of $64bitAppUninstallName failed with error: $_."
		...
		# Else, log results from check.
		Else {
			Write-Log -Message "Previous MSI versions of $64bitAppUninstallName are not installed."
		}
